...
public class ConfigureController extends Controller {
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		programs_shortcut_changed(null);
	}	
		...
		for(Node node : components.getChildrenUnmodifiable()) {
			...
			if(ci.is_checked()) {
				if(ci.get_component() instanceof LaunchComponent) {
					...
					lc.set_autostart(autostart_shortcut_cb.isSelected());
					lc.set_desktop(desktop_shortcut_cb.isSelected());
					lc.set_applications(programs_shortcut_cb.isSelected());
					lc.set_url_assoc(url_assoc_cb.isSelected());
				}
				_tmp.add(ci.get_component());
			}
		}

		FXMLLoader loader = new FXMLLoader(getClass().getResource("/ru/pp/w5277c/yaji/views/Install.fxml"));
		Parent root = loader.load();
	...
	@FXML
	private void programs_shortcut_changed(ActionEvent event) {
		if(programs_shortcut_cb.isSelected()) {
			url_assoc_cb.setDisable(false);
		}
		else {
			url_assoc_cb.setDisable(true);
			url_assoc_cb.setSelected(false);
		}
	}
}